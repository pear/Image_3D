...
//        printf("Hit polygon %d at %s (scan_depth: %d; transparency: %.2f; reflection: %.2f)\n", $nearest, $cuttingPoint->__toString(), $this->_depth - $depth + 1, $transparency, $reflection);
        ...
        if ($reflection > 0) {
            ...
            $reflectionRay = new Image_3D_Line(
                $cuttingPoint->getX(), 
                $cuttingPoint->getY(), 
                $cuttingPoint->getZ(), 
                $direction->sub($normale->multiply($normale->scalar($direction))->multiply(2)) // l - 2n (n * l)
            );
